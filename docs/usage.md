...
```
  Common options:
    ...
    -chart <type>        The type of chart:
                         horizontal - index values horizontal and
                         plot values vertical,
                         pie - a grid of pie chart plots or,
                         vertical - a list of values, like a bar
                         chart only vertical.
...
```
    ...
    corresponding to the rows or columns.
    Negative numbers forms the upper bound on a range, '1 -4' is
    the same as '1 2 3 4'. The first number is initially 1 so in
    fact '-4' suffices.
    The word 'all' means all i.e. '1 -64', only on the command
    line though.
